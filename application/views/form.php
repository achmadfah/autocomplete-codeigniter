...
<body>
			...
			<td>
				<select name="provinsi" id="provinsi" style="width: 200px;">
					...
					foreach($provinsi as $data){ // Lakukan looping pada variabel siswa dari controller
						echo "<option value='".$data->id."'>".$data->pillar_title."</option>";
					}
					?>
				</select>
			</td>
	...
	$(document).ready(function(){ // Ketika halaman sudah siap (sudah selesai di load)
		...
		$("#provinsi").change(function(){ // Ketika user mengganti atau memilih data provinsi
			$("#kota").hide(); // Sembunyikan dulu combobox kota nya // Sembunyikan dulu combobox kota nya
			$("#loading").show(); // Tampilkan loadingnya
		
			$.ajax({
				type: "POST", // Method pengiriman data bisa dengan GET atau POST
				url: "<?php echo base_url("index.php/form/listKota"); ?>", // Isi dengan url/path file php yang dituju
				data: {pillar_id : $("#provinsi").val()}, // data yang akan dikirim ke file yang dituju
				dataType: "json",
		...
		$("#kota").change(function(){ // Ketika user mengganti atau memilih data provinsi
			$("#kecamatan").hide(); // Sembunyikan dulu combobox kota nya // Sembunyikan dulu combobox kota nya
			$("#loading").show(); // Tampilkan loadingnya
		
			$.ajax({
				type: "POST", // Method pengiriman data bisa dengan GET atau POST
				url: "<?php echo base_url("index.php/form/listkecamatan"); ?>", // Isi dengan url/path file php yang dituju
				data: {parent_id : $("#kota").val()}, // data yang akan dikirim ke file yang dituju
				dataType: "json",
				beforeSend: function(e) {
					if(e && e.overrideMimeType) {
						e.overrideMimeType("application/json;charset=UTF-8");
